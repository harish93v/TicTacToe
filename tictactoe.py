...
# Retrieved from:
# http://en.literateprograms.org/Tic_Tac_Toe_(Python)?oldid=17009
...
def humanPlayer(board, player):
    ...
    move = raw_input("Enter your move (%s): " %
...
def computerPlayer(board, player):
    ...
    def evaluateMove(move, p=player):
        try:
            ...
            outcomes = (evaluateMove(next_move, opponent[p])
                        for next_move in board.getValidMoves())
            if p == player:
                # return min(outcomes)
                min_element = 1
                for o in outcomes:
